...
const shortPosition: OverlayTemplate = {
  ...
  createPointFigures: ({ coordinates, bounding, overlay, chart }) => {
    ...
    const riskH = stop.y - entry.y
    if (riskH > 0) {
      figures.push({ type: 'rect', attrs: { x: 0, y: entry.y, width: bounding.width, height: riskH }, styles: { style: 'fill', color: 'rgba(239, 83, 80, 0.12)' }, ignoreEvent: true })
    }
    if (target) {
      lines.push({ coordinates: [{ x: 0, y: target.y }, { x: bounding.width, y: target.y }] })
      texts.push({ x: 4, y: target.y, text: `Target ${(overlay.points[2]?.value ?? 0).toFixed(pp)}`, baseline: 'top' })
      const rewardH = entry.y - target.y
      if (rewardH > 0) {
        figures.push({ type: 'rect', attrs: { x: 0, y: target.y, width: bounding.width, height: rewardH }, styles: { style: 'fill', color: 'rgba(38, 166, 154, 0.12)' }, ignoreEvent: true })
        if (riskH > 0) {
          texts.push({ x: bounding.width - 80, y: entry.y + 4, text: `R:R ${(rewardH / riskH).toFixed(2)}`, baseline: 'top' })
        }
      }
    }
    return figures
