...
- Release record: the supported headless package ships in `0.4.1-beta.23`; downstream consumers record its exact source commit and registry integrity.
...
- Release record: the hardened headless package ships in `0.4.1-beta.23`; downstream consumers must complete exact-artifact acceptance.
